...
```
  ...
  _run kubectl create configmap hdfs-k8s-1670341739-krb5-config  \
...
```
  ...
  $ _HOSTS+=$(khdfs describe configmap hdfs-k8s-1670442053-config |  \
...
```
  $ _SECRET_CMD="kubectl -n my-hdfs create secret generic hdfs-k8s-1670442053-krb5-keytabs"
  $ _ktutil=""
  $ for _HOST in $_HOSTS; do
      kubectl -n my-hdfs exec $_KDC -- kadmin.local -q  \
        "addprinc -randkey hdfs/$_HOST@MYCOMPANY.COM"
      kubectl -n my-hdfs exec $_KDC -- kadmin.local -q  \
        "addprinc -randkey HTTP/$_HOST@MYCOMPANY.COM"
      kubectl -n my-hdfs exec $_KDC -- kadmin.local -q  \
        "addprinc -randkey hive/$_HOST@MYCOMPANY.COM"  
      kubectl -n my-hdfs exec $_KDC -- kadmin.local -q  \
        "ktadd -norandkey -k /tmp/$_HOST.keytab hdfs/$_HOST@MYCOMPANY.COM HTTP/$_HOST@MYCOMPANY.COM hive/$_HOST@MYCOMPANY.COM"
      kubectl -n my-hdfs cp $_KDC:/tmp/$_HOST.keytab $_MY_DIR/tmp/$_HOST.keytab
      _ktutil+="read_kt $_MY_DIR/tmp/$_HOST.keytab\n"
    done
    _ktutil+="write_kt /tmp/hdfs.keytab\n"
    printf "%b" "$_ktutil" | ktutil
    _SECRET_CMD+=" --from-file=/tmp/hdfs.keytab"
```
