...
```
  ...
  _run kubectl create configmap hdfs-k8s-1669714558-krb5-config  \
...
```
  $ _SECRET_CMD="kubectl -n my-hdfs create secret generic hdfs-k8s-1669714558-krb5-keytabs"
  $ for _HOST in $_HOSTS; do
      ...
      kubectl -n my-hdfs exec $_KDC -- kadmin.local -q  \
        "addprinc -randkey root/$_HOST@MYCOMPANY.COM"  
      kubectl -n my-hdfs exec $_KDC -- kadmin.local -q  \
        "ktadd -norandkey -k /tmp/$_HOST.keytab hdfs/$_HOST@MYCOMPANY.COM HTTP/$_HOST@MYCOMPANY.COM"
      kubectl -n my-hdfs cp my-hdfs/$_KDC:/tmp/$_HOST.keytab $_MY_DIR/tmp/$_HOST.keytab
